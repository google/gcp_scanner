...
# Defining of credentials and tokens
...
service-accounts/default/email"
    ...
    try:
        ...
        if not res.ok:
            logging.error(
                "Failed to retrieve instance token. Status code %d", res.status_code
            )
            return None, None
        ...
        if not res.ok:
            logging.error(
                "Failed to retrieve instance scopes. Status code %d", res.status_code
            )
            return None, None
        ...
        if not res.ok:
            logging.error(
                "Failed to retrieve instance email. Status code %d", res.status_code
            )
            return None, None
...
def impersonate_sa(
    iam_client: IAMCredentialsClient, target_account: str
) -> Credentials:
    ...
    return credentials_from_token(
        intermediate_access_token.access_token, None, None, None, None, scopes_sa
    )
...
def creds_from_access_token(access_token_file):
    ...
    return credentials_from_token(
        creds_dict["access_token"], None, None, None, None, user_scopes
    )

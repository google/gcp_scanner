...
# Giving Heading by assigning a variable
...
def print_diff(f1, f2):
    ...
    for line in difflib.unified_diff(
        file_1_text, file_2_text, fromfile=f1, tofile=f2, lineterm=""
    ):
        print(line)
        res += line
...
def test_creds_fetching():
    ...
    data_value = ("test_account@gmail.com", "ya.29c.TEST", valid_tm, "test", "test2")
    c.execute(sqlite_insert_with_param, data_value)
    data_value = ("test_account2@gmail.com", "ya.29c.TEST", expired_tm, "test", "test2")
    c.execute(sqlite_insert_with_param, data_value)
    conn.commit()

    assert (
        str(credsdb.get_access_tokens_dict("./unit/credentials.db"))
...
creds='test_data', token='ya.29c.TEST')]"
    ...
    print(str(res))
    assert (
        str(credsdb.get_account_creds_list("./unit"))
        == "[[SA(account_name='test_account@gmail.com', \
creds='test_data', token='ya.29c.TEST')]]"
    )
...
class TestScopes(unittest.TestCase):
    ...
    def test_get_scope_from_rt(self, mocked_post):
        ...
        mocked_post.return_value = Mock(
            status_code=201, json=lambda: {"scope": scope_str}
        )
    ...
    def test_get_scope_from_rt_no_scope(self, mocked_post):
        ...
        # Empty JSON returned
        mocked_post.return_value = Mock(status_code=201, json=lambda: {})
...
class TestCrawler(unittest.TestCase):
    ...
    def test_services(self):
        ...
        self.assertTrue(
            verify(
                crawl.list_services(PROJECT_NAME, self.credentials), "services", True
            )
        )

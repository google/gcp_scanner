...
# importing all the modules and library
import json
import logging
import os
import sys
...
# Defining function to configure
...
def crawl_loop(
    initial_sa_tuples: List[Tuple[str, Credentials, List[str]]],
    out_dir: str,
    scan_config: Dict,
    ...
    while not context.service_account_queue.empty():
        ...
        for project in project_list:
            ...
            if is_set(scan_config, "gke_clusters"):
                ...
                project_result["gke_clusters"] = crawl.get_gke_clusters(
                    project_id, gke_client
                )
            if is_set(scan_config, "gke_images"):
                project_result["gke_images"] = crawl.get_gke_images(
            ...
            if impers is not None and impers.get("impersonate", False) is True:
                ...
                for candidate_service_account in project_service_accounts:
                    ...
                    try:
                        ...
                        logging.error(
                            "Failed to get token for %s", candidate_service_account
                        )
            ...
            with open(
                out_dir + "/%s.json" % project_id, "a", encoding="utf-8"
            ) as outfile:
...
def iam_client_for_credentials(credentials: Credentials) -> IAMCredentialsClient:
    return iam_credentials.IAMCredentialsClient(credentials=credentials)


def compute_client_for_credentials(credentials: Credentials) -> discovery.Resource:
...
def main():
    ...
    crawl_loop(
        sa_tuples, args.output, scan_config, args.target_project, force_projects_list
    )
    return 0

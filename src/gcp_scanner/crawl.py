...
def fetch_project_info(project_name: str, credentials: Credentials) -> Dict[str, Any]:
...
def get_compute_instances_names(
    project_name: str, service: discovery.Resource
) -> List[Dict[str, Any]]:
...
def get_subnets(
    project_name: str, compute_client: discovery.Resource
) -> List[Dict[str, Any]]:
...
def get_bucket_names(
    project_name: str, credentials: Credentials, dump_fd: io.TextIOWrapper
...
def get_managed_zones(
    project_name: str, credentials: Credentials
) -> List[Dict[str, Any]]:
...
def get_bq_tables(
    project_id: str, dataset_id: str, bq_service: discovery.Resource
) -> List[Dict[str, Any]]:
...
def get_bq(
    project_id: str, credentials: Credentials
...
def get_pubsub_subscriptions(
    ...
    try:
        ...
        request = (
            service.projects().subscriptions().list(project=f"projects/{project_id}")
        )
        while request is not None:
            response = request.execute()
...
def get_bigtable_instances(
    ...
    try:
        ...
        request = service.projects().instances().list(parent=f"projects/{project_id}")
        while request is not None:
            response = request.execute()
        ...
        logging.info("Failed to retrieve BigTable instances for project %s", project_id)
        logging.info(sys.exc_info())
...
def get_spanner_instances(
    ...
    try:
        ...
        logging.info("Failed to retrieve Spanner instances for project %s", project_id)
        logging.info(sys.exc_info())
    ...
    try:
        ...
        for location_id in locations_list:
            request_loc = (
                ...
                .locations()
                .keyRings()
                .list(parent=f"projects/{project_id}/locations/{location_id}")
            )
...
def get_app_services(project_name: str, credentials: Credentials) -> Dict[str, Any]:
...
def get_endpoints(project_id: str, credentials: Credentials) -> List[Dict[str, Any]]:
    ...
    try:
        service = discovery.build(
            "servicemanagement", "v1", credentials=credentials, cache_discovery=False
        )
...
def get_service_accounts(
    ...
    try:
        ...
        while request is not None:
            response = request.execute()
            service_accounts = [
                (service_account["email"], service_account.get("description", ""))
...
def list_sourcerepo(project_id: str, credentials: Credentials) -> List[Any]:
    ...
    request = (
        service.projects().repos().list(name="projects/" + project_id, pageSize=500)
    )
    try:
        while request is not None:
            response = request.execute()
...
def list_dns_policies(project_id: str, credentials: Credentials) -> List[Any]:
    ...
    request = service.policies().list(project=project_id, maxResults=500)
    try:
        while request is not None:
            response = request.execute()
